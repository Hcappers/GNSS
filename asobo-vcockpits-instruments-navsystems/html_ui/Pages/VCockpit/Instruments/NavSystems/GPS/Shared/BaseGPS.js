class BaseGPS extends NavSystem {
    ...
    parseXMLConfig() {
        ...
        if (this.instrumentXmlConfig) {
            ...
            let deviceElem = this.instrumentXmlConfig.getElementsByTagName("DeviceIndex");
            if (deviceElem.length > 0) {
                this.deviceIndex = parseInt(deviceElem[0].textContent);
            }
    ...
    onUpdate(_deltaTime) {
        ...
        if (!this.newCDIBehavior) {
            const CDIState = !SimVar.GetSimVarValue("GPS DRIVES NAV1", "boolean");
            diffAndSetText(this.CDIState, CDIState ? "VLOC" : "GPS");
        }
        ...
        if (this.flightPlanManager.getWaypointsCount() > 0) {
            var distance = this.flightPlanManager.getDistanceToActiveWaypoint();
            if (this.flightPlanManager.isActiveApproach()) {
                this.currentMode = 3;
                diffAndSetText(this.botRadioPartMid, "APPR");
            }
            else if (this.flightPlanManager.getWaypointsCount() === (this.flightPlanManager.getActiveWaypointIndex() + 1) && distance <= 30) {
                if (distance <= 10) {
                    this.currentMode = 3;
                    diffAndSetText(this.botRadioPartMid, "APPR");
                }
...
class GPS_ComNav extends NavSystemElement {
    ...
    onEnter() {
        this.gps.flightPlanManager.updateFlightPlan(() => {
            this.airportListOnPlan = this.gps.flightPlanManager.getWaypoints().filter(wp => { return wp.icao[0] === "A"; });
...
class GPS_Position extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        ...
        if (reference.icao) {
            let bearing = 0;
            let distance = Avionics.Utils.computeGreatCircleDistance(reference.coordinates, new LatLong(SimVar.GetSimVarValue("GPS POSITION LAT", "degree latitude"), SimVar.GetSimVarValue("GPS POSITION LON", "degree longitude")));
            if (this.referenceMode == 0) {
                bearing = Avionics.Utils.computeGreatCircleHeading(new LatLong(SimVar.GetSimVarValue("GPS POSITION LAT", "degree latitude"), SimVar.GetSimVarValue("GPS POSITION LON", "degree longitude")), reference.coordinates);
            }
            else {
                bearing = Avionics.Utils.computeGreatCircleHeading(reference.coordinates, new LatLong(SimVar.GetSimVarValue("GPS POSITION LAT", "degree latitude"), SimVar.GetSimVarValue("GPS POSITION LON", "degree longitude")));
            }
            diffAndSetText(this.positionRefBearing, fastToFixed(bearing, 0));
            diffAndSetText(this.positionRefDistance, fastToFixed(distance, 0));
        }
        else {
...
class GPS_AirportWaypointApproaches extends NavSystemElement {
    ...
    loadApproachIntoFPL() {
        this.gps.flightPlanManager.setApproachIndex(this.selectedApproach, () => {
    ...
    loadApproachAndActivate() {
        this.gps.flightPlanManager.setApproachIndex(this.selectedApproach, () => {
            this.gps.flightPlanManager.activateApproach(() => {
                this.gps.SwitchToMenuName("FPL");
                this.gps.SwitchToInteractionState(0);
            });
        }, this.selectedTransition);
...
class GPS_NearestAirports extends NavSystemElement {
    ...
    init() {
        ...
        this.airportsSliderGroup = new SelectableElementSliderGroup(this.gps, [], this.sliderElement, this.sliderCursorElement, 1, "", 0, 1);
    ...
    onUpdate(_deltaTime) {
        ...
        for (var i = 0; i < this.nearestAirportList.airports.length; i++) {
            var firstLine;
            var secondLine;
            var logo = this.nearestAirportList.airports[i].imageFileName();
            firstLine = [
                this.nearestAirportList.airports[i].ident,
                '<img src="/Pages/VCockpit/Instruments/Shared/Map/Images/' + logo + '" class="imgSizeS">',
                fastToFixed(this.nearestAirportList.airports[i].bearing, 0) + '<div class="Align unit">o<br />M</div>',
                fastToFixed(this.nearestAirportList.airports[i].distance, 1) + '<div class="Align unit">n<br />m</div>',
                this.nearestAirportList.airports[i].bestApproach
            ];
            secondLine = [
                this.nearestAirportList.airports[i].frequencyName,
                fastToFixed(this.nearestAirportList.airports[i].frequencyMHz, 3),
                'rwy',
                fastToFixed(this.nearestAirportList.airports[i].longestRunwayLength, 0) + '<div class="Align unit">f<br />t</div>',
                ""
            ];
            dataElems.push(firstLine);
            dataElems.push(secondLine);
        }
        this.airportsSliderGroup.setDataElements(dataElems);
    }
    onExit() {
...
class GPS_NearestIntersection extends NavSystemElement {
    ...
    init() {
        ...
        this.intersectionsSliderGroup = new SelectableElementSliderGroup(this.gps, [], this.sliderElement, this.sliderCursorElement, 1, "", 0);
        for (let i = 0; i < this.nbElemsMax; i++) {
    ...
    onUpdate(_deltaTime) {
        ...
        for (var i = 0; i < this.nearestIntersectionList.intersections.length; i++) {
            var data = [
                this.nearestIntersectionList.intersections[i].ident,
                '<img src="/Pages/VCockpit/Instruments/Shared/Map/Images/' + this.nearestIntersectionList.intersections[i].imageFileName() + '">',
                fastToFixed(this.nearestIntersectionList.intersections[i].bearing, 0) + '<div class="Align unit">o<br />M</div>',
                fastToFixed(this.nearestIntersectionList.intersections[i].distance, 1) + '<div class="Align unit">n<br />m</div>',
                ""
            ];
            datas.push(data);
        }
        this.intersectionsSliderGroup.setDataElements(datas);
    }
    onExit() {
        if (this.gps.currentInteractionState == 1) {
...
class GPS_NearestNDB extends NavSystemElement {
    ...
    init() {
        ...
        this.ndbsSliderGroup = new SelectableElementSliderGroup(this.gps, [], this.sliderElement, this.sliderCursorElement, 1, "", 0);
        for (let i = 0; i < this.nbElemsMax; i++) {
    ...
    onUpdate(_deltaTime) {
        ...
        for (var i = 0; i < this.nearestNDBList.ndbs.length; i++) {
            ...
            dataElems.push([
                ...
                fastToFixed(this.nearestNDBList.ndbs[i].frequencyMHz, 2)
            ]);
        }
        this.ndbsSliderGroup.setDataElements(dataElems);
    }
    onExit() {
        if (this.gps.currentInteractionState == 1) {
...
class GPS_NearestVOR extends NavSystemElement {
    ...
    init() {
        ...
        this.vorsSliderGroup = new SelectableElementSliderGroup(this.gps, [], this.sliderElement, this.sliderCursorElement, 1, "", 0);
        for (let i = 0; i < this.nbElemsMax; i++) {
            this.vorsSliderGroup.addElement(new SelectableElement(this.gps, this.gps.getChildById("NRST_VOR_" + i), this.vor_SelectionCallback.bind(this)));
        }
    ...
    onUpdate(_deltaTime) {
        ...
        for (let i = 0; i < this.nearestVORList.vors.length; i++) {
            ...
            dataElems.push([
                ...
                fastToFixed(infos.bearing, 0) + "°",
                fastToFixed(infos.distance, 1) + "NM",
                fastToFixed(this.nearestVORList.vors[i].frequencyMHz, 2)
            ]);
        }
        this.vorsSliderGroup.setDataElements(dataElems);
    }
    onExit() {
        if (this.gps.currentInteractionState == 1) {
...
class GPS_DirectTo extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        ...
        if (infos && infos.icao != '') {
            ...
            if (this.crs) {
                let bearing = Avionics.Utils.computeGreatCircleHeading(new LatLong(SimVar.GetSimVarValue("GPS POSITION LAT", "degree latitude"), SimVar.GetSimVarValue("GPS POSITION LON", "degree longitude")), infos.coordinates);
                diffAndSetText(this.crs, fastToFixed(bearing, 0));
            }
        }
        else {
        ...
        this.icaoSearchField.Update();
        let waypoints = this.gps.flightPlanManager.getWaypoints();
        if (this.currentFPLWpSelected < waypoints.length) {
            diffAndSetText(this.fpl, waypoints[this.currentFPLWpSelected].GetInfos().ident);
        }
    }
    onExit() {
    }
    ...
    flightPlan_SelectionCallback(_event) {
        if (_event == "ENT_Push" || _event == "RightSmallKnob_Right" || _event == "RightSmallKnob_Left") {
            var elements = [];
            for (var i = 0; i < this.gps.flightPlanManager.getWaypoints().length; i++) {
                elements.push(new ContextualMenuElement(this.gps.flightPlanManager.getWaypoints()[i].GetInfos().ident, function (_index) {
                    this.currentFPLWpSelected = _index;
                    this.icaoSearchField.SetWaypoint(this.gps.flightPlanManager.getWaypoints()[_index].type, this.gps.flightPlanManager.getWaypoints()[_index].GetInfos().icao);
                    this.gps.SwitchToInteractionState(0);
                }.bind(this, i)));
            }
            if (this.gps.flightPlanManager.getWaypoints().length > 0) {
