...
class NavSystemTouch_ScrollElement {
    ...
    update() {
        ...
        if (this.isScrollLocked) {
            if (this.isMouseDragging && this.frameWithoutMovement > 6) {
...
class NavSystemTouch_DuplicateWaypointSelection extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        ...
        let planePosition = new LatLong(SimVar.GetSimVarValue("PLANE LATITUDE", "degree"), SimVar.GetSimVarValue("PLANE LONGITUDE", "degree"));
        this.wayPoints.sort((wp1, wp2) => {
            if (wp1 && wp2 && wp1.infos && wp1.infos.coordinates && wp2.infos && wp2.infos.coordinates) {
                let distance1 = Avionics.Utils.computeGreatCircleDistance(planePosition, wp1.infos.coordinates);
                let distance2 = Avionics.Utils.computeGreatCircleDistance(planePosition, wp2.infos.coordinates);
                return (distance1 - distance2);
            }
            if (wp1 && wp2 && wp1.ident && wp2.ident) {
                return wp1.ident.localeCompare(wp2.ident);
            }
            return 0;
        });
        for (let i = 0; i < this.wayPoints.length; i++) {
            ...
            else if (infos.city != "" && infos.region != "") {
                diffAndSetText(this.lines[i].identButton_Name, "");
                diffAndSetText(this.lines[i].identButton_City, infos.city + ", " + infos.region);
            }
            else if (infos.city != "" || infos.region != "") {
                diffAndSetText(this.lines[i].identButton_Name, "");
                diffAndSetText(this.lines[i].identButton_City, infos.city != "" ? infos.city : infos.region);
            }
            ...
            if (infos.coordinates) {
                let bearing = fastToFixed(Avionics.Utils.computeGreatCircleHeading(planePosition, infos.coordinates), 1);
                let distance = fastToFixed(Avionics.Utils.computeGreatCircleDistance(planePosition, infos.coordinates), 1);
                diffAndSetAttribute(this.lines[i].bearingArrow, "style", "transform: rotate(" + bearing + "deg)");
                diffAndSetText(this.lines[i].bearingText, bearing + "°");
                diffAndSetText(this.lines[i].distance, distance + "NM");
            }
        }
    }
    onExit() {
...
class NavSystemTouch_DirectTo extends NavSystemElement {
    ...
    onLoadEnd() {
        if (this.CurrentWaypoint) {
            ...
            this.DRCT_Distance.innerHTML = Avionics.Utils.computeDistance(new LatLong(SimVar.GetSimVarValue("PLANE LATITUDE", "degree", this.gps.instrumentIdentifier), SimVar.GetSimVarValue("PLANE LONGITUDE", "degree", this.gps.instrumentIdentifier)), infos.coordinates).toFixed(1) + "NM";
            this.DRCT_Bearing.innerHTML = "42°";
        }
    }
    onExit() {
    }
...
class NavSystemTouch_ActiveFPL extends NavSystemElement {
    constructor(_smallArrow, overrideButton) {
        super();
        ...
        this._overrideButton = overrideButton;
    }
    ...
    init(root) {
        ...
        if (this._overrideButton && this._overrideButton.drctCallback) {
            this.gps.makeButton(this.drct_Button, () => {
                this._overrideButton.drctCallback(this.selectedElement ? this.selectedElement.index : undefined);
            });
        }
        else {
            this.gps.makeButton(this.drct_Button, this.directTo.bind(this));
    ...
    addEnrouteEndKeyboard(_icao) {
        ...
        position += this.gps.currFlightPlanManager.getEnRouteWaypoints(null).length;
