...
class SelectableElementSliderGroup extends SelectableElement {
    constructor(_gps, _elements, _slider, _cursor, _step = 1, _emptyLine = "", _blinkingElementIndex = 1, _lineBreak = 0) {
        ...
        this.blinkingElementIndex = _blinkingElementIndex;
        ...
        if (l > 0) {
            ...
            if (!result) {
                switch (_event) {
                    case "NavigationLargeInc":
                        do {
                            if (this.index == this.elements.length - 1 - this.lineBreak) {
                                if ((this.index + this.offset) < l - 1 - this.lineBreak) {
                                    this.offset += (this.step + this.lineBreak);
                                ...
                                if (this.index < Math.min(this.elements.length, l) - 1 - this.lineBreak) {
                                    this.index++;
                                    this.index += this.lineBreak;
                                    result = true;
                                }
                            }
    ...
    onSelection(_event) {
        ...
        if (_event == "NavigationLargeInc") {
            ...
            this.index = Math.min(l, this.elements.length) - 1 - this.lineBreak;
        }
        if (this.dataElements.length > this.stringElements.length) {
            this.updateDisplayWithData();
        }
        ...
        for (let i = 0; i < Math.min(this.elements.length, this.dataElements.length); i++) {
            ...
            for (let j = 0; j < 5; j++) {
                ...
                if (!cell) {
                    ...
                    if (j === this.blinkingElementIndex) {
