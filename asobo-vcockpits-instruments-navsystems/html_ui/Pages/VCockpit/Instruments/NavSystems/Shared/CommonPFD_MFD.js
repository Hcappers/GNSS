...
class PFD_Attitude extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        ...
        if (xyz) {
            ...
            diffAndSetAttribute(this.svg, "low-bank-mode", SimVar.GetSimVarValue("AUTOPILOT MAX BANK", "degree"));
        }
    }
    onExit() {
    }
    ...
    onUpdate(_deltaTime) {
        ...
        if (infos && (infos.icao != "" || infos.ident != "")) {
            ...
            if (infos.coordinates && infos.coordinates.lat && infos.coordinates.long) {
                ...
                let distance = Avionics.Utils.computeGreatCircleDistance(new LatLong(SimVar.GetSimVarValue("GPS POSITION LAT", "degree latitude"), SimVar.GetSimVarValue("GPS POSITION LON", "degree longitude")), infos.coordinates);
        ...
        if (info && info.icao != "") {
            ...
            if (info.coordinates) {
                ...
                if (info.coordinates.lat && info.coordinates.long) {
                    let bearing = Avionics.Utils.computeGreatCircleHeading(new LatLong(SimVar.GetSimVarValue("GPS POSITION LAT", "degree latitude"), SimVar.GetSimVarValue("GPS POSITION LON", "degree longitude")), info.coordinates);
                    let distance = Avionics.Utils.computeGreatCircleDistance(new LatLong(SimVar.GetSimVarValue("GPS POSITION LAT", "degree latitude"), SimVar.GetSimVarValue("GPS POSITION LON", "degree longitude")), info.coordinates);
    ...
    elementSelectionCallback(_event, _index) {
        if (_event == "ENT_Push") {
            switch (this.menuIndex) {
                case 0:
                    let waypoints = this.gps.flightPlanManager.getWaypoints();
                    this.drctElement.icaoSearchField.SetWaypoint(waypoints[_index].type, waypoints[_index].GetInfos().icao);
                    break;
                case 1:
                    break;
                case 2:
