...
class AS3000_TSC extends NavSystemTouch {
    ...
    connectedCallback() {
        ...
        if (SimVar.GetSimVarValue("L:AS3000_IsLocalVarDefined", "boolean") == 0) {
...
class AS3000_TSC_LightingConfigs extends NavSystemElement {
    ...
    updateCursor() {
        let backLight = SimVar.GetSimVarValue("L:AS3000_Brightness", "number");
        diffAndSetText(this.masterPercentText, fastToFixed((backLight * 100), 0));
        ...
        this.masterCursor.style.left = (this.cursorStartVH + (this.cursorBGWidthVH * backLight)) + "vh";
    }
    onLessPress() {
        SimVar.SetSimVarValue("L:AS3000_Brightness", "number", Math.max(0, Math.min(1, SimVar.GetSimVarValue("L:AS3000_Brightness", "number") - 0.01)));
    }
    onMorePress() {
        SimVar.SetSimVarValue("L:AS3000_Brightness", "number", Math.max(0, Math.min(1, SimVar.GetSimVarValue("L:AS3000_Brightness", "number") + 0.01)));
    }
    ...
    mouseMove(event) {
        if (this.isCursorMoving) {
            ...
            if (!isNaN(pos)) {
                SimVar.SetSimVarValue("L:AS3000_Brightness", "number", pos);
            }
        }
    }
}
class AS3000_TSC_NavComHome extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        ...
        diffAndSetAttribute(this.XPDRIdent, "state", SimVar.GetSimVarValue("TRANSPONDER IDENT:1", "bool") ? "Active" : "");
...
class AS3000_TSC_WaypointOptions extends NavSystemElement {
    ...
    insertInFplIndexSelectionCallback(_index) {
        this.gps.flightPlanManager.addWaypoint(this.icao, _index, () => {
            this.gps.SwitchToPageName("MFD", "Active Flight Plan");
        });
    }
    showInMapToggle() {
