class AS3X_Touch extends NavSystemTouch {
    ...
    connectedCallback() {
            ...
            new AS3X_Touch_PageGroup("MFD", this, [
                ...
                new AS3X_Touch_NavSystemPage("Active FPL", "FPL", new NavSystemElementGroup([
                    new NavSystemTouch_ActiveFPL(true, {
                        drctCallback: (selectedWaypointIndex) => {
                            this.lastRelevantICAO = this.flightPlanManager.getWaypoints()[selectedWaypointIndex].icao;
                            this.lastRelevantICAOType = this.flightPlanManager.getWaypoints()[selectedWaypointIndex].icao.charAt(0);
        ...
        if (SimVar.GetSimVarValue("L:AS3X_Touch_IsLocalVarDefined", "boolean") == 0) {
            SimVar.SetSimVarValue("L:AS3X_Touch_Brightness", "number", 1);
            SimVar.SetSimVarValue("L:AS3X_Touch_Brightness_IsAuto", "boolean", true);
        }
    }
    parseXMLConfig() {
        ...
        switch (this.displayMode) {
            ...
            case "Splitted":
                ...
                if (this.urlConfig.style === "largeenginedisplay") {
                    let mfdElement = document.getElementById("MFD");
                    diffAndSetAttribute(mfdElement, "displaySize", "bigDisplay");
                    let engineInfosElement = document.getElementById("EngineInfos");
                    engineInfosElement.style.width = "19%";
                }
                break;
        }
...
class AS3X_Touch_Setup_Display extends NavSystemElement {
    ...
    init(root) {
        ...
        if (SimVar.GetSimVarValue("L:AS3X_Touch_Brightness_IsAuto", "boolean")) {
            this.switchToAuto();
        }
        else {
            this.switchToManual();
    ...
    updateCursor() {
        let brightness = SimVar.GetSimVarValue("L:AS3X_Touch_Brightness", "number");
        diffAndSetText(this.masterPercentText, fastToFixed((brightness * 100), 0));
        let length = brightness * 100;
        let height = brightness * 30;
        diffAndSetAttribute(this.masterBGTriangle, "points", "0,30 " + length + "," + (30 - height) + " " + length + ",30");
        this.masterCursor.style.left = (this.cursorStartVH + (this.cursorBGWidthVH * brightness)) + "vh";
    }
    onLessPress() {
        if (!SimVar.GetSimVarValue("L:AS3X_Touch_Brightness_IsAuto", "boolean")) {
            SimVar.SetSimVarValue("L:AS3X_Touch_Brightness", "number", Math.max(0, Math.min(1, SimVar.GetSimVarValue("L:AS3X_Touch_Brightness", "number") - 0.01)));
        }
    }
    onMorePress() {
        if (!SimVar.GetSimVarValue("L:AS3X_Touch_Brightness_IsAuto", "boolean")) {
    ...
    mouseMove(event) {
        if (this.isCursorMoving && !SimVar.GetSimVarValue("L:AS3X_Touch_Brightness_IsAuto", "boolean")) {
            let pos = Math.max(0, Math.min(1, (event.clientX - this.leftX) / this.widthX));
            if (!isNaN(pos)) {
                SimVar.SetSimVarValue("L:AS3X_Touch_Brightness", "number", pos);
            }
        }
    }
    openModePopup() {
    ...
    switchToAuto() {
        ...
        SimVar.SetSimVarValue("L:AS3X_Touch_Brightness_IsAuto", "boolean", true);
        this.switchCursorInteraction(true);
    }
    switchToManual() {
        diffAndSetAttribute(this.modePopup, "state", "Inactive");
        SimVar.SetSimVarValue("L:AS3X_Touch_Brightness_IsAuto", "boolean", false);
        this.switchCursorInteraction(false);
    }
    switchCursorInteraction(mode) {
        diffAndSetAttribute(this.masterCursor, "state", mode ? "Greyed" : "");
        diffAndSetAttribute(this.buttonLess, "state", mode ? "Greyed" : "");
        diffAndSetAttribute(this.buttonMore, "state", mode ? "Greyed" : "");
        diffAndSetText(this.buttonMode_Value, mode ? "Photo Cell" : "Manual");
    }
}
